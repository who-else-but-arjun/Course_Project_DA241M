...
def preprocess_image(image, image_shape=(256, 256)):
    ...
    img = cv2.resize(image, image_shape[:2])  # Resize to (256, 256)

    # Check if the image has 3 channels; if not, convert it to RGB
    if len(img.shape) == 2:  # Grayscale image
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
    elif img.shape[2] == 1:  # If image has a single channel
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
    ...
    image = img_to_array(img)  # Convert image to array
...
def deblur_image(image, weights_path='Weights_for_DeblurGAN.h5'):
...
if __name__ == '__main__':
    ...
    weights_path = 'Weights_for_DeblurGAN.h5'
